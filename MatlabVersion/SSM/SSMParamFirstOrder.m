function [Cp,Sol_FULL,Rhs_FULL,Mat_FULL,Tri,AY,XTA] = SSMParamFirstOrder(SSMParams,MatParams,ForceVector, Cp,freq,Phi,K,M,C)
    disp(["Order 1"])
    Rhs_FULL = zeros(SSMParams.nMat,1);
    Sol_FULL = zeros(SSMParams.nMat,1);
    Mat_FULL= sparse(SSMParams.nMat,SSMParams.nMat);
    ...
    omega0 = zeros(SSMParams. nm,1); 
    elta0 = zeros(SSMParams.nm,1);
