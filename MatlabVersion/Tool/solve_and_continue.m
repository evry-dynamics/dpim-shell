...
function [X,Solinfo,Sol,stable,FloquetExponents,Bifurcation] = ...
    solve_and_continue(x0,fun_residual,lam_s,lam_e,ds,system,varargin)
...
Sopt = struct('flag',1,'predictor','tangent', ...
    ...
    'reversaltolerance',.1,...
    'stepmax',2e3,'eps',1e-4,'noconv_stepcor','red','errmax',3,...
    'epsFD',1e-7,...
...
solopt_tmp = Solopt;
Solopt = optimset(solopt_tmp,'MaxIter',2000,'Display','iter');
...
while istep<=Sopt.stepmax
    ...
    Xold = X0;
    [Rold,dRdXold] = feval(fun_residual,diag(Sopt.Dscale)*Xold);
    if istep==2
        Bifurcation{istep-1} = 'No Bifurcation';
    end
    ...
    [Rnew,dRdXnew] = feval(fun_residual,diag(Sopt.Dscale)*X0);
    
    % Save solution point
    X(:,istep) = diag(Sopt.Dscale)*X0;
    [stable,FloquetExponents,BifurcationType]=stable_analysis(dRdXold,dRdXnew,system,stable, FloquetExponents,istep);
    Bifurcation{istep} = BifurcationType;
